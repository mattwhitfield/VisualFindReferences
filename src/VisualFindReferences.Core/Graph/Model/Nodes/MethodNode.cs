...
{
    public class MethodNode : VFRNode
    {
        ...
        public MethodNode(NodeGraph flowChart, FoundReferences foundReferences) : base(flowChart, foundReferences, DefaultIcon, Brushes.MediumOrchid)
        { }

        public override string NodeSymbolType => "Method";

        public override IEnumerable<SearchableSymbol> GetSearchableSymbols()
        {
            yield return new SearchableSymbol(NodeFoundReferences, new[] { NodeFoundReferences.Symbol }, NodeFoundReferences.Solution, "method " + ContainerName);
        }
    }
}